...
type TeamMemberResponse = PcoResponse<{
  team_position_name: string;
}>;

type NeededPositionResponse = PcoResponse<{
  name: string;
}>;

type PlanResponse = PcoResponse<{
  short_dates: string;
...
async function getRosterByServiceType(
  serviceTypeId: string,
  opts?: { offset?: string }
) {
  const plans = await httpClient.get<PlanResponse>(
  ...
  const [teamMembers, neededPositions] = await Promise.all([
    httpClient.get<TeamMemberResponse>(
      plans.data.data[0].links.self + "/team_members",
      {
        params: {
          include: "team"
        }
    ...
    httpClient.get<NeededPositionResponse>(
      plans.data.data[0].links.self + "/needed_positions",
      {
        params: {
          include: "team"
        }
  ...
  return {
    ...
    pcoUrl: plans.data.data[0]?.attributes.planning_center_url
  };
}
...
exports.handler = async (event: {
  ...
  const servicesFilter = event.queryStringParameters?.services
    ?.split(",")
  ...
  let serviceTypes: Awaited<ReturnType<typeof getRosterByServiceType>>[];
  ...
  for (const serviceType of serviceTypes) {
    for (const team of [
      ...serviceType.teamMembers.included,
      ...serviceType.neededPositions.included
    ]) {
